...
None. Every item that used to be on this list is now resolved — see "Resolved since last pass" below. The one thing still genuinely open, and unresearchable (not an external-spec lookup, a design call with no official source): whether the `-disabled` blend percentage (A2, 50% toward neutral, our own default) and the MD2 elevation dp best-fit (A9, 1/4/8/16/24dp, drawn from MD2's component table rather than a single named scale) hold up once real components get built against them — both are reasonable starting defaults, not verified-correct the way the sourced numbers below are.
