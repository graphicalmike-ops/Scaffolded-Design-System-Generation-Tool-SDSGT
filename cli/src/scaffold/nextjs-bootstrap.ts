...
// `_variables.scss` sitting next to it (Sass partial convention).
...
function writeThemeCss(appDir: string, families: { primary: string; secondary?: string }, fontFaceCss: string): void {
  ...
  writeFileSync(join(appDir, "theme.css"), `${parts.join("\n\n")}\n`, "utf-8");
}
...
function writeReadme(outDir: string, projectName: string): void {
  const content = [
    ...
    "  overrides (colors, border-radius). Re-run SDSGT's `generate` step to",
    "  update these; don't hand-edit them directly, since the next scaffold",
    "  run overwrites them.",
...
export function scaffoldNextjsBootstrap(opts: ScaffoldNextjsBootstrapOptions): ScaffoldResult {
  ...
  // was passed) globals.css is superseded by globals.scss below — remove it
  // rather than leave an unimported, dead file sitting in the project.
  const defaultGlobalsCss = join(appDir, "globals.css");
  if (existsSync(defaultGlobalsCss)) rmSync(defaultGlobalsCss);

  writeFileSync(join(appDir, "_variables.scss"), readFileSync(variablesScssPath, "utf-8"), "utf-8");
  filesWritten.push("src/app/_variables.scss");
  ...
  writeThemeCss(appDir, families, fontFaceCss);
