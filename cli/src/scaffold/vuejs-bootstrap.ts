...
function writeThemeCss(assetsDir: string, families: { primary: string; secondary?: string }, fontFaceCss: string): void {
  ...
  writeFileSync(join(assetsDir, "theme.css"), `${parts.join("\n\n")}\n`, "utf-8");
}
...
export function scaffoldVuejsBootstrap(opts: ScaffoldVuejsBootstrapOptions): ScaffoldResult {
  ...
  writeFileSync(join(assetsDir, "_variables.scss"), readFileSync(variablesScssPath, "utf-8"), "utf-8");
  filesWritten.push("src/assets/_variables.scss");
  ...
  writeThemeCss(assetsDir, families, fontFaceCss);
